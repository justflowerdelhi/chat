...
export function getDemoCatalogueRecommendations(message: string): DemoCatalogueProduct[] {
  ...
  if (budget) {
    const aroundBudget = /\\b(around|about|near|approx|approximately)\\b/i.test(lower);

    if (aroundBudget) {
      const inRange = products.filter(
        (product) =>
          product.price >= Math.max(0, budget - 300) &&
          product.price <= budget + 300
      );
      ...
      const withinBudget = products.filter((product) => product.price <= budget);
      products = withinBudget.length
        ? withinBudget
        : products.sort(
            (a, b) =>
              Math.abs(a.price - budget) - Math.abs(b.price - budget)
  ...
  const occasionProducts = products.filter((product) =>
    occasionMatches(product, message)
  );

  if (occasionProducts.length > 0) {
    products = [
      ...occasionProducts,
      ...products.filter((product) => !occasionProducts.includes(product)),
    ];
  }
