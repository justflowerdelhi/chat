...
Do not create orders, confirm payment, confirm delivery or invent values.`,
    ...
    // Enquiry already exists for this conversation.
    // Return the existing summary so the webhook can notify the florist.
    ...
    if (existingResult.rows[0]?.summary) {
      const existingSummary = existingResult.rows[0].summary;

      if (typeof existingSummary === "object") {
        return existingSummary as EnquirySummary;
      }

      try {
        return JSON.parse(existingSummary) as EnquirySummary;
      } catch {
        return summary;
      }
    }
...
export function extractKnownDetails(history: ChatHistoryRow[]): KnownConversationDetails {
  ...
  for (const msg of userMessages) {
    ...
    if (!details.budget) {
      const budgetMatch = text.replace(/,/g, '').match(/(?:â‚¹|rs\.?|inr)?\s*(\d{3,6})(?:\s*(?:rupees|rs|inr))?/i);
      if (budgetMatch) details.budget = `â‚¹${Number(budgetMatch[1]).toLocaleString('en-IN')}`;
    }
...
function buildStateInstruction(details: KnownConversationDetails): string {
  ...
  return [
    'CURRENT CUSTOMER STATE â€” AUTHORITATIVE CONVERSATION MEMORY:',
...
export async function runReceptionConversation({
  ...
  const shouldEnforceUsageLimit = enforceUsageLimit !== false;
  ...
  let usageCheck: { allowed: boolean; remaining: number } | null = null;

  if (shouldEnforceUsageLimit) {
...
`
  ...
  const enquiry = await createEnquiryIfCompleted(currentSessionId, historyMessages, reply);
  ...
  return {
    ...
    remaining: shouldEnforceUsageLimit && usageCheck ? usageCheck.remaining - 1 : 0,
    enquiry,
    status: 200 as const,
  };
}
